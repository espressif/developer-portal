...
To check the effect of allocating memory from PSRAM we can also compare the memory report provided at the end of build process after linking phase in both situations:

```
$ west build -b esp32s3_devkitc/esp32s3/procpu zephyr/samples/net/wifi/shell --pristine
```

```
Memory region         Used Size  Region Size  %age Used
           FLASH:      614364 B    8388352 B      7.32%
     iram0_0_seg:       72696 B     343552 B     21.16%
     dram0_0_seg:      147936 B     327168 B     45.22%
     irom0_0_seg:      345490 B         8 MB      4.12%
     drom0_0_seg:      483292 B         8 MB      5.76%
    rtc_iram_seg:          0 GB         8 KB      0.00%
    rtc_data_seg:          0 GB         8 KB      0.00%
    rtc_slow_seg:          0 GB         8 KB      0.00%
        IDT_LIST:          0 GB         8 KB      0.00%
...
```
$ west build -b esp32s3_devkitc/esp32s3/procpu zephyr/samples/net/wifi/shell --pristine -DCONFIG_ESP_SPIRAM=y -DCONFIG_SPIRAM_TYPE_ESPPSRAM64=y -DCONFIG_SPIRAM_MODE_OCT=y -DCONFIG_SPIRAM_SPEED_80M=y -DCONFIG_ESP32S3_DATA_CACHE_64KB=y -DCONFIG_NET_DEBUG_NET_PKT_ALLOC=y -DCONFIG_SPIRAM_FETCH_INSTRUCTIONS=y -DCONFIG_SPIRAM_RODATA=y

```

```
Memory region         Used Size  Region Size  %age Used
           FLASH:      616112 B    8388352 B      7.34%
     iram0_0_seg:       77016 B     343552 B     22.42%
     dram0_0_seg:      155136 B     327168 B     47.42%
     irom0_0_seg:      349078 B         8 MB      4.16%
     drom0_0_seg:      485040 B         8 MB      5.78%
    ext_dram_seg:       1536 KB         8 MB     18.75%
...
***>>>>>***Some work still need to be done here to finishing***<<<<<***
