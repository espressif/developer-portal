...
As of the time of writing, although ESP32, ESP32-S2, ESP32-S3, ESP32-C61, and ESP32-P4 support external PSRAM, ESP32-C61 and ESP32-P4 are not currently supported on Zephyr yet.
...
The are limits regarding the maximum PSRAM capacities that each ESP32 SoC series can manage. As an example, ESP32-S3 can manage PSRAMs up to 32 MB.
...
This configuration parameter enables support for an external SPI RAM chip, connected in parallel with the main SPI flash chip. If enabled, it automatically enables [SHARED_MULTI_HEAP](#6-dynamically-allocating-psram-memory).

### 4.2. ESP_HEAP_SEARCH_ALL_REGIONS

This configuration parameter enables searching all available heap regions. If the region of desired capability is exhausted, memory will be allocated from another available regions.
...
This configuration parameter selects the mode of SPI RAM chip in use. The permitted values are `SPIRAM_MODE_QUAD` and `SPIRAM_MODE_OCT`. Please note that SPIRAM_MODE_OCT is only available in ESP32-S3.

### 4.6. SPIRAM_TYPE

This configuration parameter defines the type of SPIRAM chip in use:
...
This configuration parameter sets the SPIRAM clock speed in MHz:
 | Value | Clock speed |
 | --- | --- |
...
This configuration parameter allows moving Instructions from flash to PSRAM. If enabled, instructions in flash will be moved into PSRAM on startup. If [SPIRAM_RODATA](#49-spiram_rodata) parameter is also enabled, the code that normally requires execution during the SPI1 flash operation does not need to be placed in IRAM, thus optimizing RAM usage. By default, this parameter is disabled.

### 4.9. SPIRAM_RODATA

This configuration parameter allows moving read-only data from flash to PSRAM. If [SPIRAM_FETCH_INSTRUCTIONS](#48-spiram_fetch_instructions) parameter is also enabled, the code that normally requires execution during the SPI1 flash operation  does not need to be placed in IRAM, thus optimizing RAM usage.
...
Once [ESP_SPIRAM](#41-esp_spiram) is enabled, a section called `.ext_ram.bss` will be created. This section will hold non-initialized global variables that will later be placed in PSRAM. These gloabal variables must be declared with `__attribute__ ((section (".ext_ram.bss"))`.
...
Here, we can observe that the application now starts earlier compared to the previous sample. This improvement results from disabling the [ESP_SPIRAM_MEMTEST](#44-esp_spiram_memtest) parameter, which bypasses the PSRAM memory test that previously took a few hundred milliseconds.

## 8. Moving code from FLASH to PSRAM

Enabling the [SPIRAM_FETCH_INSTRUCTIONS](#48-spiram_fetch_instructions) parameter will move instructions from FLASH to PSRAM during startup, and if the [ESP_SPIRAM_MEMTEST](#44-esp_spiram_memtest) parameter is also enabled, code that requires execution during an SPI1 Flash operation can forgo being placed in IRAM, thus optimizing RAM usage.

To check the effects of [SPIRAM_FETCH_INSTRUCTIONS](#48-spiram_fetch_instructions) and [SPIRAM_RODATA](#49-spiram_rodata), let's build `hello_world` first without enabling such parameters and then enable them.

### 8.1. Building `hello_world` without `SPIRAM_FETCH_INSTRUCTIONS` and `SPIRAM_RODATA` enabled
...
### 8.2. Building `hello_world` with `SPIRAM_FETCH_INSTRUCTIONS` and `SPIRAM_RODATA` enabled`
...
In addition to these three strategies, it is also possible to leverage PSRAM to execute external code compiled as Position Independent Code (PIC), which can be placed in PSRAM by a loader for execution.

Another potential use of PSRAM to reduce SRAM usage is by enabling `ESP32_WIFI_NET_ALLOC_SPIRAM` parameter, allowing the ESP32 Wi-Fi stack to dynamically allocate memory from PSRAM. However, as of the time of writing, there is an open [issue](https://github.com/zephyrproject-rtos/zephyr/issues/74675#issuecomment-2531361367) preventing applications built with `ESP32_WIFI_NET_ALLOC_SPIRAM` parameter enabled from functioning properly.
