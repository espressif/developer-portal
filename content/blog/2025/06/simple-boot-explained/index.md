...
tags:
  ...
  - Zephyr
  - NuttX
summary: "A simplified ESP32 boot process using single-image binaries can speed up builds and flashing, ideal for development use. It trades off features like OTA updates and encryption for faster iteration."
---
...
The ESP32 family of microcontrollers traditionally relies on a second-stage bootloader. Due to the design of the ROM loader, using this bootloader is typically the only viable method to boot the SoC.
...
To satisfy these constraints, the second-stage bootloader is essential. Among other functions, it serves as a bridge to overcome the limitations imposed by the ROM loader.
...
The ESP image format is described in detail in the [chapter of related article](https://developer.espressif.com/blog/esp32-bootstrapping/#rom-loader), which outlines how the ROM loader reads and processes each segment during boot.
...
To produce a compliant binary image named `zephyr.bin` from the imput ELF file `zephyr.elf`, the following command can be used:
...
After collecting all the "invisible" segments, their storage addresses (LMAs) must be corrected, as the `elf2image` tool may have reordered the segments in the binary. At this point, these segments are mapped into their correct memory locations, and the system can proceed with the standard initialization process.
...
flowchart LR
	...
	SRAM  .->|3.Map| B
	FLASH ==>|3a.Map| CACHE
	SRAM  .->|4.Platform init|CACHE 
	PSRAM ==>|3b.Map| CACHE
	CACHE .->|5.System init| INIT 
